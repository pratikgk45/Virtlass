...
def update_marks(request,pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect('classroom:student_marks_list', pk=marks.student.user.pk)
    else:
...
class add_student(LoginRequiredMixin,generic.RedirectView):
    ...
    def get(self,request,*args,**kwargs):
        ...
        try:
            ...
            messages.warning(self.request,'warning, Student is already in the class !')
        else:
...
def update_assignment(request,id=None):
    ...
    if form.is_valid():
        ...
        obj.save()
        messages.success(request, "Assigment Successfully Updated".format(obj.assignment_name))
...
def assignment_delete(request, id=None):
    ...
    if request.method == "POST":
        obj.delete()
        messages.success(request, "Assignment Successfully Removed")
        return redirect('classroom:assignment_list')
    context = {
        "object": obj,
    }
